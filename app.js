...
const Jalaali = {
    ...
    // ✅ اصلاح آرایه اعداد فارسی (۳ و ۷ جا افتاده بود)
    ...
    parseJalali(str) { if (!str || typeof str !== 'string') return null; const p = String(str).split('/').map(p => +String(p).replace(/[۰-۹]/g, d => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d))); if (p.length !== 3 || p.some(isNaN)) return null; return { year: p[0], month: p[1], day: p[2] }; },
    daysBetween(j1, j2) { if(!j1||!j2) return 0; try { const g1 = libToGregorian(j1.year, j1.month, j1.day); const d1 = new Date(g1.gy, g1.gm-1, g1.gd); const g2 = libToGregorian(j2.year, j2.month, j2.day); const d2 = new Date(g2.gy, g2.gm-1, g2.gd); return Math.ceil((d2.getTime() - d1.getTime()) / 86400000); } catch (e) { return 0; } },
    isLeapJalaali(jy) { return libIsLeapYear(jy); }, jMonthLength(jy, jm) { return libMonthLength(jy, jm); },
...
function calculateProjectStatus(p) {
    const t = Jalaali.today(); const c = Jalaali.parseJalali(p.createdAt); 
    if (!c) return { status: 'active', daysLeft: 10, daysUntilDelivery: 0, daysSinceCreation: 0, currentPeriod: 1 };
    const d = Jalaali.parseJalali(p.deliveryDate); 
    const ds = Jalaali.daysBetween(c, t); 
    const dd = d ? Jalaali.daysBetween(t, d) : 0;
    
    if (p.completed) return { status: 'delivered', daysLeft: 0, currentPeriod: 0, daysUntilDelivery: dd, daysSinceCreation: ds };
    ...
    if (rp.includes(cp)) return { status: 'active', daysLeft: daysLeftInPeriod, currentPeriod: cp, daysUntilDelivery: dd, daysSinceCreation: ds };
    if (lp < cp - 1) return { status: 'delayed', daysLeft: daysLeftInPeriod, currentPeriod: cp, daysUntilDelivery: dd, delayText: `${Jalaali.toPersianDigits(cp - 1 - lp)} دوره جا افتاده`, daysSinceCreation: ds, isLate: true };
    if (daysLeftInPeriod <= 0) return { status: 'delayed', daysLeft: 0, currentPeriod: cp, daysUntilDelivery: dd, delayText: `${Jalaali.toPersianDigits(Math.abs(daysLeftInPeriod))} روز تاخیر در گزارش`, isLate: true, delayDays: Math.abs(daysLeftInPeriod), daysSinceCreation: ds };
    
    return { status: daysLeftInPeriod <= 3 ? 'pending' : 'active', daysLeft: daysLeftInPeriod, currentPeriod: cp, daysUntilDelivery: dd, daysSinceCreation: ds, isLate: false };
}
...
function renderProjectCard(project) {
    ...
    else if (status.daysUntilDelivery < 0) { timeText = `${Jalaali.toPersianDigits(Math.abs(status.daysUntilDelivery))} روز تاخیر در تحویل`; timeBadgeClass = 'bg-red-50 dark:bg-red-500/10 text-red-600 border-red-100'; }
    else if (status.daysUntilDelivery === 0) { timeText = 'تحویل امروز'; timeBadgeClass = 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 border-amber-100'; }
    else { timeText = `${Jalaali.toPersianDigits(status.daysUntilDelivery)} روز تا تحویل`; timeBadgeClass = status.daysUntilDelivery <= 7 ? 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 border-amber-100' : 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 border-blue-100'; }
            ...
            <div class="p-5 pt-0 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3 text-xs">
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
                        <p class="text-slate-400 mb-1 text-[10px]">روزهای سپری‌شده</p>
                        <p class="font-black text-slate-900 dark:text-white text-lg">${Jalaali.toPersianDigits(status.daysSinceCreation)} روز</p>
                    </div>
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
                        <p class="text-slate-400 mb-1 text-[10px]">${status.daysUntilDelivery < 0 ? 'تاخیر از تحویل' : 'روزهای باقی‌مانده'}</p>
                        <p class="font-black ${status.daysUntilDelivery < 0 ? 'text-red-600' : status.daysUntilDelivery <= 7 ? 'text-amber-600' : 'text-emerald-600'} text-lg">${status.daysUntilDelivery < 0 ? Jalaali.toPersianDigits(Math.abs(status.daysUntilDelivery)) + ' روز' : Jalaali.toPersianDigits(status.daysUntilDelivery) + ' روز'}</p>
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-[11px] font-black ${isDelayed ? 'text-red-500' : 'text-primary-600'}">${status.daysLeft > 0 ? `${Jalaali.toPersianDigits(status.daysLeft)} روز مانده از دوره` : 'پایان دوره'}</span>
                    </div>
...
// ✅ اصلاح رویدادهای تب‌ها
document.querySelectorAll('.filter-btn').forEach(b => {
    b.addEventListener('click', () => {
        currentFilter = b.dataset.filter;
        document.querySelectorAll('.filter-btn').forEach(x => x.classList.remove('bg-white', 'text-slate-900', 'shadow-sm'));
