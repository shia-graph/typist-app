...
const Jalaali = {
    ...
    parseJalali(str) { 
        ...
        const p = String(str).split('/').map(p => +String(p).replace(/[۰-۹]/g, d => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d))); 
        if (p.length !== 3 || p.some(isNaN)) return null; 
        return { year: p[0], month: p[1], day: p[2] }; 
    },
    
    // ✅ اصلاح: محاسبه دقیق با علامت درست
    daysBetween(j1, j2) { 
        if(!j1||!j2) return 0; 
        try {
...
// ✅ اصلاح: تنظیم Event Listener برای فیلترها بعد از لود DOM
document.addEventListener('DOMContentLoaded', () => {
    ...
    filterBtns.forEach(b => {
        b.addEventListener('click', () => { 
            ...
            // اضافه کردن کلاس‌های جدید به دکمه فعال
    ...
    // تنظیم دکمه "همه" به‌عنوان پیش‌فرض
...
function openReportModal(id) { 
    ...
    if (s.status === 'delayed' && s.isLate) { 
        ...
        sb.innerHTML = `<span>این گزارش <strong>${Jalaali.toPersianDigits(s.delayDays)} روز</strong> تاخیر دارد</span>`; 
...
function exportToCSV() { 
    ...
    const h = ['نام', 'شماره', 'عنوان', 'تحویل', 'وضعیت', 'صفحات', 'گزارش‌ها']; 
