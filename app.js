...
async function loadProjects() {
    ...
    projectsCache = (data || []).map(p => {
        ...
        return { 
            ...
            startDate: p.delivery_date, // ✅ تغییر نام متغیر به startDate
...
function calculateProjectStatus(p) {
    ...
    const daysSinceStart = Math.max(0, Jalaali.daysBetween(startDate, t));
    ...
    if (Jalaali.daysBetween(t, startDate) > 0) {
...
function renderProjectCard(project) {
    ...
    else if (status.status === 'delayed' && status.isLate) { timeText = status.delayText || 'تاخیر در گزارش'; timeBadgeClass = 'bg-red-50 dark:bg-red-500/10 text-red-600 border-red-100'; }
